...
1. Переходим по ссылке на репозиторий

2. В верхнем правом углу нажимаем кнопку FORK 

    (Копируем репозиторий к себе на GitHub)

3. Создаем пустую папку на локальном компьютере

4. Командой  
> git clone ссылка на свой репозиторий
копируем репозиторий на локальный компьютер
5. Создаем и сохраняем изменения
6. Комайндой 
> git push 
сохраняем изменения у себя на GitHub
7. После обновления страницы появляется кнопка pull request, через которую предагаем свои изменения
