...
**Чтобы предложить изменения к чужому репозиторию:**

1. Fork его репозиторий себе на Github
2. `git clone` его к себе на локальный репозиторий
3. Создаем новую ветку, в которой вносим свои изменения
4. `git push` изменения на свой Github (терминал предложит ввести `git push --set-upstream origin des` - соглашаемся)
5. Жмём зеленую кнопку “Compare & pull request”
