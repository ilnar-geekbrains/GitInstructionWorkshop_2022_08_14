...
Для создания локального репозитория необходимо открыть в Git папку, в которой планируется дальнейшая работа, запустить для неё терминал и ввести в нём команду __*git init*__. После ввода данной команды в Вашей папке появится скрытая папка .git, которая и позволит в дальнейшем работать с файлами.
Внесённые в репозитории файлы необходимо добавить к отслеживанию, это выполняется командой __*git add*__. После проведения данной процедуры нужно закоммитить изменения командой __*git commit -m "Initial commit"*__. 
При последующих изменениях в файлах можно использовать упрощённую версию команды: __*git commit -am "commit name"*__

Также можно создать локальный репозиторий из удалённого, используя команду __*git clone*__, после которой необходимо вставить ссылку (URL-адрес клонирования, выбирается либо своя папка в github, либо форкнутая) на удалённый репозиторий.
