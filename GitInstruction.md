...
Посмотреть существующие ветки можно с помощью команды *git branch*. Текущая ветка, в которой мы находимся на текущий момент будет помечена символом (*) и выделена цветом

Новая ветка создается с помощью команды **git branch имя_ветки**, где *имя_ветки* - название ветки, которую мы хотим создать

Переход между ветками осуществляется с помощью команды **git checkout имя_ветки**, где *имя ветки* - название ветки, на котороую хотим переключиться.
