...
Нужна для дробления общего объема работ, распределения между програмистами в команде. Либо коллективного написания программ с открытым исходным кодом.
* В первом случае при работе в одном реппозитории каждый програмист создает свою ветку ***(branch)***, в ней пишет код, создает свои коммиты ***(commit)***. После при приемке работ производится объединение ***(merge)** побочной ветки с основной веткой.
* Во втором случае создается форк ***(fork)*** желаемого репозитория, в нем создается отдельная ветка **(branch)***, в ней пишет код, создает свои коммиты ***(commit)***.
После этого из репозитория програмиста оправляется пул реквест ***(pull request)*** в основной репозиторий.
